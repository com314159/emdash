...
const PrGenerationService_1 = require("../services/PrGenerationService");
const DatabaseService_1 = require("../services/DatabaseService");
const prIssueFooter_1 = require("../lib/prIssueFooter");
const prCreateBodyPlan_1 = require("../lib/prCreateBodyPlan");
const prIssueFooterPatch_1 = require("../lib/prIssueFooterPatch");
const remoteProjectResolver_1 = require("../utils/remoteProjectResolver");
const RemoteGitService_1 = require("../services/RemoteGitService");
...
const remoteStatusPollers = new Map();
const ensureRemoteStatusPoller = (taskPath, connectionId) => {
...
const releaseRemoteStatusPoller = (taskPath, watchId) => {
 ...
 */
function validateTaskPath(taskPath) {
    if (!taskPath)
        return 'Missing taskPath';
    if (!node_path_1.default.isAbsolute(taskPath))
        return 'taskPath must be absolute';
    try {
        const stat = node_fs_1.default.statSync(taskPath);
        if (!stat.isDirectory())
            return 'taskPath is not a directory';
    }
    catch {
...
const broadcastGitStatusChange = (taskPath, error) => {
    ...
    windows.forEach((window) => {
        try {
            ...
            logger_1.log.debug('[git:watch-status] failed to send status change', err);
        }
    });
};
const ensureGitStatusWatcher = (taskPath) => {
    if (!supportsRecursiveWatch) {
        return { success: false, error: 'recursive-watch-unsupported' };
    }
    if (!taskPath || !node_fs_1.default.existsSync(taskPath)) {
    ...
    try {
        const watcher = node_fs_1.default.watch(taskPath, { recursive: true }, () => {
        ...
        watcher.on('error', (error) => {
            logger_1.log.warn('[git:watch-status] watcher error', error);
            const entry = gitStatusWatchers.get(taskPath);
            if (entry?.debounceTimer)
                clearTimeout(entry.debounceTimer);
            try {
...
const releaseGitStatusWatcher = (taskPath, watchId) => {
    const entry = gitStatusWatchers.get(taskPath);
    if (!entry)
        return { success: true };
    if (watchId) {
...
function registerGitIpc() {
    ...
    async function commitAndPushRemote(connectionId, taskPath, opts) {
        const { commitMessage, createBranchIfOnDefault, branchPrefix } = opts;
        // Verify git repo
        const verifyResult = await remoteGitService.execGit(connectionId, taskPath, 'rev-parse --is-inside-work-tree');
        ...
        // Read staged files
        const readRemoteStagedFiles = async () => {
        ...
        await remoteGitService.execGit(connectionId, taskPath, 'reset -q PLANNING.md 2>/dev/null || true');
        await remoteGitService.execGit(connectionId, taskPath, 'reset -q planning.md 2>/dev/null || true');
    ...
    async function getPrStatusRemote(connectionId, taskPath) {
        const queryFields = [
            'number',
            'url',
            'state',
        ...
        const viewResult = await remoteGitService.execGh(connectionId, taskPath, `pr view --json ${fieldsStr} -q .`);
        let data = viewResult.exitCode === 0 && viewResult.stdout.trim()
        ...
        if (!data) {
            ...
            if (branch) {
                const listResult = await remoteGitService.execGh(connectionId, taskPath, `pr list --head ${quoteGhArg(branch)} --json ${fieldsStr} --limit 1`);
        ...
        if (asNumber(data.additions) === null ||
            ...
            if (diffResult.exitCode === 0) {
                const m = (diffResult.stdout || '').match(/(\d+)\s+files? changed(?:,\s+(\d+)\s+insertions?\(\+\))?(?:,\s+(\d+)\s+deletions?\(-\))?/);
                if (m) {
                    if (asNumber(data.changedFiles) === null && m[1])
                        data.changedFiles = parseInt(m[1], 10);
                    if (asNumber(data.additions) === null && m[2])
                        data.additions = parseInt(m[2], 10);
                    if (asNumber(data.deletions) === null && m[3])
                        data.deletions = parseInt(m[3], 10);
                }
            }
        }
    ...
    async function createPrRemote(connectionId, taskPath, opts) {
        const { title, body, base, head, draft, web, fill } = opts;
        const outputs = [];
        // Enrich body with issue footer
        let prBody = body;
        try {
            const task = await DatabaseService_1.databaseService.getTaskByPath(taskPath);
            prBody = (0, prIssueFooter_1.injectIssueFooter)(body, task?.metadata);
        }
        catch {
            // Non-fatal
        }
        const { shouldPatchFilledBody, shouldUseBodyFile: _unused, shouldUseFill, } = (0, prCreateBodyPlan_1.getCreatePrBodyPlan)({
            fill,
            title,
            rawBody: body,
            enrichedBody: prBody,
        ...
        const aheadResult = await remoteGitService.execGit(connectionId, taskPath, `rev-list --count origin/${quoteGhArg(baseRef)}..HEAD`);
        ...
        const flags = [];
        if (title)
            flags.push(`--title ${quoteGhArg(title)}`);
        // Can't use --body-file on remote, use --body instead
        if (prBody && !shouldUseFill)
            flags.push(`--body ${quoteGhArg(prBody)}`);
        ...
        if (shouldUseFill)
            flags.push('--fill');
        const createResult = await remoteGitService.execGh(connectionId, taskPath, `pr create ${flags.join(' ')}`);
        ...
        if (shouldPatchFilledBody && url) {
            try {
                const task = await DatabaseService_1.databaseService.getTaskByPath(taskPath);
                if (task?.metadata) {
                    const editBody = (0, prIssueFooter_1.injectIssueFooter)(undefined, task.metadata);
                    if (editBody) {
                        await remoteGitService.execGh(connectionId, taskPath, `pr edit --body ${quoteGhArg(editBody)}`);
                    }
                }
            }
            catch {
    ...
    async function mergeToMainRemote(connectionId, taskPath) {
        const currentBranch = await remoteGitService.getCurrentBranch(connectionId, taskPath);
        const defaultBranch = await remoteGitService.getDefaultBranchName(connectionId, taskPath);
        if (!currentBranch) {
        ...
        // Create PR
        let prUrl = '';
        const createResult = await remoteGitService.execGh(connectionId, taskPath, `pr create --fill --base ${quoteGhArg(defaultBranch)}`);
        ...
        try {
            const task = await DatabaseService_1.databaseService.getTaskByPath(taskPath);
            if (task?.metadata) {
                const footer = (0, prIssueFooter_1.injectIssueFooter)(undefined, task.metadata);
                if (footer) {
                    await remoteGitService.execGh(connectionId, taskPath, `pr edit --body ${quoteGhArg(footer)}`);
                }
            }
        }
        catch {
    ...
    const getLocalAheadBehind = async (taskPath) => {
        try {
    ...
    const getLocalPrForDeleteRisk = async (taskPath) => {
        const queryFields = [
            'number',
            'url',
            'state',
    ...
    electron_1.ipcMain.handle('git:watch-status', async (_, taskPath) => {
        const remoteProject = await (0, remoteProjectResolver_1.resolveRemoteProjectForWorktreePath)(taskPath);
        if (remoteProject) {
            return ensureRemoteStatusPoller(taskPath, remoteProject.sshConnectionId);
        }
        return ensureGitStatusWatcher(taskPath);
    });
    electron_1.ipcMain.handle('git:unwatch-status', async (_, taskPath, watchId) => {
        const remoteProject = await (0, remoteProjectResolver_1.resolveRemoteProjectForWorktreePath)(taskPath);
        if (remoteProject) {
    ...
    electron_1.ipcMain.handle('git:get-status', async (_, taskPath) => {
        try {
            const remoteProject = await (0, remoteProjectResolver_1.resolveRemoteProjectForWorktreePath)(taskPath);
            if (remoteProject) {
                const changes = await remoteGitService.getStatusDetailed(remoteProject.sshConnectionId, taskPath);
                return { success: true, changes };
            }
            const changes = await (0, GitService_1.getStatus)(taskPath);
            return { success: true, changes };
        }
        catch (error) {
    ...
    electron_1.ipcMain.handle('git:get-delete-risks', async (_, args) => {
        ...
        try {
            const entries = await Promise.all(targets.map(async (target) => {
                ...
                try {
                    const remoteProject = await (0, remoteProjectResolver_1.resolveRemoteProjectForWorktreePath)(target.taskPath);
                    const [changesRes, aheadBehindRes, prRes] = await Promise.all([
                        (async () => {
                            if (remoteProject) {
                                return await remoteGitService.getStatusDetailed(remoteProject.sshConnectionId, target.taskPath);
                            }
                            return await (0, GitService_1.getStatus)(target.taskPath);
                        })(),
                        (async () => {
                            if (remoteProject) {
                                const status = await remoteGitService.getBranchStatus(remoteProject.sshConnectionId, target.taskPath);
                        ...
                        (async () => {
                            if (!includePr)
                                return { pr: null, prKnown: false, error: undefined };
                            if (remoteProject) {
                                const result = await getPrStatusRemote(remoteProject.sshConnectionId, target.taskPath);
                                if (!result.success) {
                                    return {
                                        pr: null,
                                        prKnown: false,
                                ...
                                return { pr: result.pr ?? null, prKnown: true, error: undefined };
                            }
                            return await getLocalPrForDeleteRisk(target.taskPath);
                        })(),
                    ]);
                    const counts = countStatusBuckets(changesRes);
    ...
    electron_1.ipcMain.handle('git:get-file-diff', async (_, args) => {
        try {
    ...
    // Git: Revert file
    electron_1.ipcMain.handle('git:revert-file', async (_, args) => {
        try {
            logger_1.log.info('Reverting file:', { taskPath: args.taskPath, filePath: args.filePath });
            const remoteProject = await (0, remoteProjectResolver_1.resolveRemoteProjectForWorktreePath)(args.taskPath);
            let result;
            if (remoteProject) {
                result = await remoteGitService.revertFile(remoteProject.sshConnectionId, args.taskPath, args.filePath);
            }
            else {
                result = await (0, GitService_1.revertFile)(args.taskPath, args.filePath);
            }
            logger_1.log.info('File operation completed:', { filePath: args.filePath, action: result.action });
            return { success: true, action: result.action };
        }
        catch (error) {
            logger_1.log.error('Failed to revert file:', { filePath: args.filePath, error });
            return { success: false, error: error instanceof Error ? error.message : String(error) };
        }
    });
    // Git: Generate PR title and description
    electron_1.ipcMain.handle('git:generate-pr-content', async (_, args) => {
        const { taskPath, base = 'main' } = args || {};
        try {
            ...
            if (remoteProject) {
                ...
                const diffResult = await remoteGitService.execGit(connId, taskPath, `diff --stat origin/${quoteGhArg(base)}...HEAD`);
                const logResult = await remoteGitService.execGit(connId, taskPath, `log --oneline origin/${quoteGhArg(base)}..HEAD`);
            ...
            try {
                ...
                if (task?.agentId) {
                    ...
                    logger_1.log.debug('Found task provider for PR generation', { taskPath, providerId });
                }
            }
            catch (error) {
                logger_1.log.debug('Could not lookup task provider', { error });
                // Non-fatal - continue without provider
            }
            const result = await PrGenerationService_1.prGenerationService.generatePrContent(taskPath, base, providerId);
            return { success: true, ...result };
        }
        catch (error) {
            logger_1.log.error('Failed to generate PR content:', error);
            return {
                success: false,
    ...
    electron_1.ipcMain.handle('git:create-pr', async (_, args) => {
        const { taskPath, title, body, base, head, draft, web, fill } = args ||
            {};
        try {
            const remoteProject = await (0, remoteProjectResolver_1.resolveRemoteProjectForWorktreePath)(taskPath);
            if (remoteProject) {
            ...
            try {
                ...
                logger_1.log.debug('Unable to enrich PR body with issue footer', { taskPath, error });
            }
            const { shouldPatchFilledBody, shouldUseBodyFile, shouldUseFill } = (0, prCreateBodyPlan_1.getCreatePrBodyPlan)({
                fill,
                title,
                rawBody: body,
                enrichedBody: prBody,
            ...
            try {
                ...
                if (statusOut && statusOut.trim().length > 0) {
                    ...
                    if (addOut?.trim())
                        outputs.push(addOut.trim());
                    if (addErr?.trim())
                        outputs.push(addErr.trim());
                    const commitMsg = 'stagehand: prepare pull request';
                    try {
                        const { stdout: commitOut, stderr: commitErr } = await execAsync(`git commit -m ${JSON.stringify(commitMsg)}`, { cwd: taskPath });
                        if (commitOut?.trim())
                            outputs.push(commitOut.trim());
                        if (commitErr?.trim())
                            outputs.push(commitErr.trim());
                    }
                    catch (commitErr) {
                ...
                logger_1.log.warn('Failed to stage/commit changes before PR:', stageErr);
            ...
            try {
                ...
                try {
                    ...
                    logger_1.log.error('Failed to push branch before PR:', pushErr2);
                    return {
                        success: false,
                        error: 'Failed to push branch to origin. Please check your Git remotes and authentication.',
                    };
            ...
            try {
                const { stdout } = await execAsync('gh repo view --json defaultBranchRef -q .defaultBranchRef.name', { cwd: taskPath });
                ...
                try {
                    const { stdout } = await execAsync('git remote show origin | sed -n "/HEAD branch/s/.*: //p"', { cwd: taskPath });
            ...
            try {
                const baseRef = base || defaultBranch;
                const { stdout: aheadOut } = await execAsync(`git rev-list --count ${JSON.stringify(`origin/${baseRef}`)}..HEAD`, { cwd: taskPath });
...
current branch '${currentBranch}' ahead of base '${baseRef}'.`,
            ...
            const flags = [];
            if (title)
                flags.push(`--title ${JSON.stringify(title)}`);
            // Use temp file for body to properly handle newlines and multiline content
            let bodyFile = null;
            if (shouldUseBodyFile && prBody) {
                try {
                    bodyFile = node_path_1.default.join(node_os_1.default.tmpdir(), `gh-pr-body-${Date.now()}-${Math.random().toString(36).substring(7)}.txt`);
                    // Write body with actual newlines preserved
                    node_fs_1.default.writeFileSync(bodyFile, prBody, 'utf8');
                    flags.push(`--body-file ${JSON.stringify(bodyFile)}`);
                }
                catch (writeError) {
                    logger_1.log.warn('Failed to write body to temp file, falling back to --body flag', {
                        writeError,
                    });
            ...
            if (base || defaultBranch)
                flags.push(`--base ${JSON.stringify(base || defaultBranch)}`);
            if (head) {
            ...
            try {
                ...
                if (bodyFile && node_fs_1.default.existsSync(bodyFile)) {
                    try {
                        node_fs_1.default.unlinkSync(bodyFile);
                    }
                    catch (unlinkError) {
                        logger_1.log.debug('Failed to delete temp body file', { bodyFile, unlinkError });
                    }
                }
            }
            ...
            if (shouldPatchFilledBody) {
                try {
                    const didPatchBody = await (0, prIssueFooterPatch_1.patchCurrentPrBodyWithIssueFooter)({
                        taskPath,
                    ...
                    logger_1.log.warn('Failed to patch PR body with issue footer after --fill create', {
                        taskPath,
                        editError,
                    });
                }
            ...
            if (restrictionRe.test(combined)) {
                code = 'ORG_AUTH_APP_RESTRICTED';
                logger_1.log.warn('GitHub org restrictions detected during PR creation');
            }
            else if (prExistsRe.test(combined)) {
                code = 'PR_ALREADY_EXISTS';
                logger_1.log.info('PR already exists for branch - push was successful');
            }
            else {
                logger_1.log.error('Failed to create PR:', combined || error);
            }
            return {
                success: false,
    ...
    electron_1.ipcMain.handle('git:get-pr-status', async (_, args) => {
        const { taskPath } = args || {};
        try {
            const remoteProject = await (0, remoteProjectResolver_1.resolveRemoteProjectForWorktreePath)(taskPath);
            if (remoteProject) {
            ...
            try {
                ...
                if (!data) {
                    try {
                        ...
                        logger_1.log.warn('Failed to fallback to gh pr list:', fallbackErr);
                        // Ignore fallback errors and return original null/error
                    }
                }
                if (!data)
                    return { success: false, error: 'No PR data returned' };
                // Fallback: if GH CLI didn't return diff stats, try to compute locally
                const asNumber = (v) => typeof v === 'number' && Number.isFinite(v)
                    ? v
                ...
                if (!hasAdd || !hasDel || !hasFiles) {
                    ...
                    try {
                        ...
                        const m = statLine &&
                            statLine.match(/(\d+)\s+files? changed(?:,\s+(\d+)\s+insertions?\(\+\))?(?:,\s+(\d+)\s+deletions?\(-\))?/);
                        if (m) {
                            const [, filesStr, addStr, delStr] = m;
                            if (!hasFiles && filesStr)
                                data.changedFiles = Number.parseInt(filesStr, 10);
                            if (!hasAdd && addStr)
                                data.additions = Number.parseInt(addStr, 10);
                            if (!hasDel && delStr)
                                data.deletions = Number.parseInt(delStr, 10);
                        }
                    }
                    catch {
    ...
    electron_1.ipcMain.handle('git:merge-pr', async (_, args) => {
        const { taskPath, prNumber, strategy = 'merge', admin = false, } = (args || {});
        try {
            const remoteProject = await (0, remoteProjectResolver_1.resolveRemoteProjectForWorktreePath)(taskPath);
            if (remoteProject) {
                ...
                if (admin)
                    ghArgs.push('--admin');
                const result = await remoteGitService.execGh(remoteProject.sshConnectionId, taskPath, ghArgs.join(' '));
    ...
    electron_1.ipcMain.handle('git:enable-auto-merge', async (_, args) => {
        const { taskPath, prNumber, strategy = 'merge', } = (args || {});
        try {
            const strategyFlag = strategy === 'squash' ? '--squash' : strategy === 'rebase' ? '--rebase' : '--merge';
            const ghArgs = ['pr', 'merge'];
            if (typeof prNumber === 'number' && Number.isFinite(prNumber)) {
    ...
    electron_1.ipcMain.handle('git:disable-auto-merge', async (_, args) => {
        const { taskPath, prNumber } = (args || {});
        try {
            const ghArgs = ['pr', 'merge'];
            if (typeof prNumber === 'number' && Number.isFinite(prNumber)) {
                ghArgs.push(String(prNumber));
    ...
    electron_1.ipcMain.handle('git:get-check-runs', async (_, args) => {
        const { taskPath } = args || {};
        try {
            const remoteProject = await (0, remoteProjectResolver_1.resolveRemoteProjectForWorktreePath)(taskPath);
            if (remoteProject) {
                ...
                const checksResult = await remoteGitService.execGh(connId, taskPath, `pr checks --json ${fields}`);
                ...
                try {
                    const shaResult = await remoteGitService.execGh(connId, taskPath, "pr view --json headRefOid --jq '.headRefOid'");
                    const sha = shaResult.stdout.trim();
                    if (sha) {
                        const apiResult = await remoteGitService.execGh(connId, taskPath, `api repos/{owner}/{repo}/commits/${sha}/check-runs --jq '.check_runs | map({name: .name, html_url: .html_url}) | .[]'`);
                        ...
                        for (const line of apiResult.stdout.trim().split('\n')) {
                            ...
                            try {
                                ...
                                if (entry.name && entry.html_url)
                                    urlMap.set(entry.name, entry.html_url);
                            }
                            catch { }
                        }
            ...
            try {
                ...
                try {
                    const { stdout: shaOut } = await execFileAsync('gh', ['pr', 'view', '--json', 'headRefOid', '--jq', '.headRefOid'], { cwd: taskPath });
                    const sha = shaOut.trim();
                    if (sha) {
                        const { stdout: apiOut } = await execFileAsync('gh', [
                            'api',
    ...
    electron_1.ipcMain.handle('git:get-pr-comments', async (_, args) => {
        const { taskPath, prNumber } = args || {};
        try {
            const remoteProject = await (0, remoteProjectResolver_1.resolveRemoteProjectForWorktreePath)(taskPath);
            if (remoteProject) {
                ...
                if (data.number) {
                    try {
                        ...
                        const setAvatar = (login, url) => {
                            avatarMap.set(login, url);
                            if (login.endsWith('[bot]'))
                                avatarMap.set(login.replace(/\[bot]$/, ''), url);
                        };
                        const commentsApi = await remoteGitService.execGh(connId, taskPath, `api repos/{owner}/{repo}/issues/${data.number}/comments --jq '.[] | {login: .user.login, avatar_url: .user.avatar_url}'`);
                        for (const line of commentsApi.stdout.trim().split('\n')) {
                            if (!line)
                                continue;
                            try {
                                const entry = JSON.parse(line);
                                if (entry.login && entry.avatar_url)
                                    setAvatar(entry.login, entry.avatar_url);
                            }
                            catch { }
                        }
                        const reviewsApi = await remoteGitService.execGh(connId, taskPath, `api repos/{owner}/{repo}/pulls/${data.number}/reviews --jq '.[] | {login: .user.login, avatar_url: .user.avatar_url}'`);
            ...
            try {
                ...
                if (data.number) {
                    try {
                        const avatarMap = new Map();
                        const { stdout: commentsApi } = await execFileAsync('gh', [
                            'api',
                        ...
                        const { stdout: reviewsApi } = await execFileAsync('gh', [
                            'api',
    ...
    electron_1.ipcMain.handle('git:commit-and-push', async (_, args) => {
        const { taskPath, commitMessage = 'chore: apply task changes', createBranchIfOnDefault = true, branchPrefix = 'orch', } = (args ||
            {});
        try {
            const remoteProject = await (0, remoteProjectResolver_1.resolveRemoteProjectForWorktreePath)(taskPath);
            if (remoteProject) {
            ...
            try {
                ...
                logger_1.log.warn('Stage/commit step issue:', e instanceof Error ? e.message : String(e));
                throw e;
            }
            ...
            logger_1.log.error('Failed to commit and push:', error);
            const errObj = error;
    ...
    electron_1.ipcMain.handle('git:get-branch-status', async (_, args) => {
        const { taskPath } = args || {};
        if (!taskPath) {
        ...
        if (remoteProject) {
            try {
                const status = await remoteGitService.getBranchStatus(remoteProject.sshConnectionId, taskPath);
                return { success: true, ...status };
            }
            catch (error) {
                logger_1.log.error(`getBranchStatus (remote): error for ${taskPath}:`, error);
                return { success: false, error: error instanceof Error ? error.message : String(error) };
            }
        }
        // Early exit for missing/invalid local path
        if (!node_fs_1.default.existsSync(taskPath)) {
            logger_1.log.warn(`getBranchStatus: path does not exist: ${taskPath}`);
            return { success: false, error: 'Path does not exist' };
        }
        ...
        try {
            ...
            logger_1.log.warn(`getBranchStatus: not a git repository: ${taskPath}`);
            return { success: false, error: 'Not a git repository' };
        }
        try {
            // Current branch
            ...
            let defaultBranch = 'main';
            try {
                const { stdout } = await execFileAsync('gh', ['repo', 'view', '--json', 'defaultBranchRef', '-q', '.defaultBranchRef.name'], { cwd: taskPath });
                const db = (stdout || '').trim();
                if (db)
                    defaultBranch = db;
            }
                ...
                try {
                    ...
                    const { stdout } = await execFileAsync(GIT, ['symbolic-ref', '--short', 'refs/remotes/origin/HEAD'], { cwd: taskPath });
            ...
            try {
                ...
                const { stdout } = await execFileAsync(GIT, ['rev-list', '--left-right', '--count', '@{upstream}...HEAD'], { cwd: taskPath });
                ...
                try {
                    ...
                    const { stdout } = await execFileAsync(GIT, ['rev-list', '--left-right', '--count', `origin/${branch}...HEAD`], { cwd: taskPath });
                    const parts = (stdout || '').trim().split(/\s+/);
                    if (parts.length >= 2) {
                        behind = parseInt(parts[0] || '0', 10) || 0;
                        ahead = parseInt(parts[1] || '0', 10) || 0;
            ...
            if (branch !== defaultBranch) {
                try {
                    const { stdout: countOut } = await execFileAsync(GIT, ['rev-list', '--count', `origin/${defaultBranch}..HEAD`], { cwd: taskPath });
            ...
            logger_1.log.error(`getBranchStatus: unexpected error for ${taskPath}:`, error);
            return { success: false, error: error instanceof Error ? error.message : String(error) };
        }
    });
    electron_1.ipcMain.handle('git:list-remote-branches', async (_, args) => {
        const { projectPath, remote = 'origin' } = args || {};
        if (!projectPath) {
            return { success: false, error: 'projectPath is required' };
        }
        const remoteProject = await (0, remoteProjectResolver_1.resolveRemoteProjectForWorktreePath)(projectPath);
        if (remoteProject) {
            try {
                const branches = await remoteGitService.listBranches(remoteProject.sshConnectionId, projectPath, remote);
                return { success: true, branches };
            }
            catch (error) {
                logger_1.log.error('Failed to list branches (remote):', error);
                return { success: false, error: error instanceof Error ? error.message : String(error) };
            }
        }
        try {
        ...
        try {
            ...
            try {
                ...
                try {
                    ...
                    logger_1.log.warn('Failed to fetch remote before listing branches', fetchError);
                }
            }
            catch {
                // Remote doesn't exist, skip fetch and will use local branches instead
                logger_1.log.debug(`Remote '${remote}' not found, will use local branches`);
            }
            let branches = [];
            if (hasRemote) {
                // List remote branches
                const { stdout } = await execFileAsync('git', ['for-each-ref', '--format=%(refname:short)', `refs/remotes/${remote}`], { cwd: projectPath });
                branches =
                    stdout
                        ?.split('\n')
                        .map((line) => line.trim())
                ...
                try {
                    const { stdout: localStdout } = await execAsync('git for-each-ref --format="%(refname:short)" refs/heads/', { cwd: projectPath });
                    const remoteBranchNames = new Set(branches.map((b) => b.branch));
                    const localOnlyBranches = localStdout
                        ?.split('\n')
                        .map((line) => line.trim())
                        .filter((line) => line.length > 0)
                    ...
                    logger_1.log.warn('Failed to list local branches', localBranchError);
                }
            }
            else {
                // No remote - list local branches instead
                try {
                    const { stdout } = await execAsync('git for-each-ref --format="%(refname:short)" refs/heads/', { cwd: projectPath });
                    branches =
                        stdout
                            ?.split('\n')
                            .map((line) => line.trim())
            ...
            logger_1.log.error('Failed to list branches:', error);
            return { success: false, error: error instanceof Error ? error.message : String(error) };
        }
    });
    // Git: Merge current branch to main via GitHub (create PR + merge immediately)
    electron_1.ipcMain.handle('git:merge-to-main', async (_, args) => {
        const { taskPath } = args || {};
        try {
            const remoteProject = await (0, remoteProjectResolver_1.resolveRemoteProjectForWorktreePath)(taskPath);
            if (remoteProject) {
            ...
            // Stage and commit any pending changes
            const { stdout: statusOut } = await execAsync('git status --porcelain --untracked-files=all', { cwd: taskPath });
            if (statusOut?.trim()) {
                await execAsync('git add -A', { cwd: taskPath });
                try {
            ...
            try {
                ...
                logger_1.log.debug('Unable to load task metadata for merge-to-main issue footer', {
                    taskPath,
                    metadataError,
                });
            }
            try {
                ...
                const errMsg = e?.stderr || String(e);
            ...
            if (prExists) {
                try {
                    await (0, prIssueFooterPatch_1.patchCurrentPrBodyWithIssueFooter)({
                        taskPath,
                        metadata: taskMetadata,
                        execFile: execFileAsync,
                        prUrl,
                    ...
                    logger_1.log.warn('Failed to patch merge-to-main PR body with issue footer', {
                        taskPath,
                        editError,
                    });
                }
            ...
            logger_1.log.error('Failed to merge to main:', e);
            return { success: false, error: e?.message || String(e) };
        }
    });
    // Git: Rename branch (local and optionally remote)
    electron_1.ipcMain.handle('git:rename-branch', async (_, args) => {
        const { repoPath, oldBranch, newBranch } = args;
        try {
            logger_1.log.info('Renaming branch:', { repoPath, oldBranch, newBranch });
            const remoteProject = await (0, remoteProjectResolver_1.resolveRemoteProjectForWorktreePath)(repoPath);
            if (remoteProject) {
                const result = await remoteGitService.renameBranch(remoteProject.sshConnectionId, repoPath, oldBranch, newBranch);
            ...
            try {
                const { stdout: remoteOut } = await execFileAsync(GIT, ['config', '--get', `branch.${oldBranch}.remote`], { cwd: repoPath });
                if (remoteOut?.trim()) {
                ...
                try {
                    const { stdout: lsRemote } = await execFileAsync(GIT, ['ls-remote', '--heads', 'origin', oldBranch], { cwd: repoPath });
                    if (lsRemote?.trim()) {
                        remotePushed = true;
                    }
                }
            ...
            logger_1.log.info('Local branch renamed successfully');
            // If pushed to remote, delete old and push new
            if (remotePushed) {
                logger_1.log.info('Branch was pushed to remote, updating remote...');
                try {
                    // Delete old remote branch
                    await execFileAsync(GIT, ['push', remoteName, '--delete', oldBranch], {
                        cwd: repoPath,
                    });
                    logger_1.log.info('Deleted old remote branch');
                }
                catch (deleteErr) {
                    // Remote branch might not exist or already deleted
                    logger_1.log.warn('Could not delete old remote branch (may not exist):', deleteErr);
                }
                // Push new branch and set upstream
                await execFileAsync(GIT, ['push', '-u', remoteName, newBranch], { cwd: repoPath });
                logger_1.log.info('Pushed new branch to remote');
            }
            return { success: true, remotePushed };
        }
        catch (error) {
            logger_1.log.error('Failed to rename branch:', error);
            return { success: false, error: error instanceof Error ? error.message : String(error) };
        }
    });
    electron_1.ipcMain.handle('git:commit', async (_, args) => {
        try {
            const pathErr = validateTaskPath(args.taskPath);
            if (pathErr)
                return { success: false, error: pathErr };
            const result = await (0, GitService_1.commit)(args.taskPath, args.message);
    ...
    electron_1.ipcMain.handle('git:push', async (_, args) => {
        try {
            const pathErr = validateTaskPath(args.taskPath);
            if (pathErr)
                return { success: false, error: pathErr };
            const result = await (0, GitService_1.push)(args.taskPath);
    ...
    electron_1.ipcMain.handle('git:pull', async (_, args) => {
        try {
            const pathErr = validateTaskPath(args.taskPath);
            if (pathErr)
                return { success: false, error: pathErr };
            const result = await (0, GitService_1.pull)(args.taskPath);
            return { success: true, output: result.output };
        }
        catch (error) {
            const errObj = error;
    ...
    electron_1.ipcMain.handle('git:get-log', async (_, args) => {
        try {
            const pathErr = validateTaskPath(args.taskPath);
            if (pathErr)
                return { success: false, error: pathErr };
            const result = await (0, GitService_1.getLog)(args.taskPath, args.maxCount, args.skip, args.aheadCount);
    ...
    electron_1.ipcMain.handle('git:get-latest-commit', async (_, args) => {
        try {
            const pathErr = validateTaskPath(args.taskPath);
            if (pathErr)
                return { success: false, error: pathErr };
            const commit = await (0, GitService_1.getLatestCommit)(args.taskPath);
    ...
    electron_1.ipcMain.handle('git:get-commit-files', async (_, args) => {
        try {
            const pathErr = validateTaskPath(args.taskPath);
            if (pathErr)
                return { success: false, error: pathErr };
            ...
            const files = await (0, GitService_1.getCommitFiles)(args.taskPath, args.commitHash);
    ...
    electron_1.ipcMain.handle('git:get-commit-file-diff', async (_, args) => {
        try {
            const pathErr = validateTaskPath(args.taskPath);
            if (pathErr)
                return { success: false, error: pathErr };
    ...
    electron_1.ipcMain.handle('git:soft-reset', async (_, args) => {
        try {
            const pathErr = validateTaskPath(args.taskPath);
            if (pathErr)
                return { success: false, error: pathErr };
            const result = await (0, GitService_1.softResetLastCommit)(args.taskPath);
            broadcastGitStatusChange(args.taskPath);

...
exports.registerTelemetryIpc = registerTelemetryIpc;
...
function registerTelemetryIpc() {
    electron_1.ipcMain.handle('telemetry:capture', async (_event, args) => {
        try {
            ...
            const props = args?.properties && typeof args.properties === 'object' ? args.properties : undefined;
            ...
            if (ev === '$exception') {
                ...
                (0, telemetry_1.captureException)(error, props);
            }
            else {
                // Regular telemetry events
                (0, telemetry_1.capture)(ev, props);
            }
            return { success: true };
        }
        catch (e) {
    ...
    electron_1.ipcMain.handle('telemetry:get-status', async () => {
        try {
            return { success: true, status: (0, telemetry_1.getTelemetryStatus)() };
        }
        catch (e) {
            return { success: false, error: e?.message || 'status_failed' };
        }
    });
    electron_1.ipcMain.handle('telemetry:set-enabled', async (_event, enabled) => {
        try {
            (0, telemetry_1.setTelemetryEnabledViaUser)(Boolean(enabled));
            return { success: true, status: (0, telemetry_1.getTelemetryStatus)() };
        }
        catch (e) {
    ...
    electron_1.ipcMain.handle('telemetry:set-onboarding-seen', async (_event, flag) => {
        try {
            (0, telemetry_1.setOnboardingSeen)(Boolean(flag));
            return { success: true, status: (0, telemetry_1.getTelemetryStatus)() };
        }
        catch (e) {
            return { success: false, error: e?.message || 'update_failed' };

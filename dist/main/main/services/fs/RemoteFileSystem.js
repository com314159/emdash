...
class RemoteFileSystem {
    constructor(sshService, connectionId, remotePath) {
        this.sshService = sshService;
        this.connectionId = connectionId;
        this.remotePath = remotePath;
        if (!sshService) {
            throw new types_1.FileSystemError('SSH service is required', types_1.FileSystemErrorCodes.CONNECTION_ERROR);
        }
        if (!connectionId) {
            throw new types_1.FileSystemError('Connection ID is required', types_1.FileSystemErrorCodes.CONNECTION_ERROR);
        }
        if (!remotePath) {
            throw new types_1.FileSystemError('Remote path is required', types_1.FileSystemErrorCodes.INVALID_PATH);
        }
     ...
     */
    async read(path, maxBytes = DEFAULT_MAX_BYTES) {
        const fullPath = this.resolveRemotePath(path);
        const sftp = await this.sshService.getSftp(this.connectionId);
        return new Promise((resolve, reject) => {
            sftp.open(fullPath, 'r', (err, handle) => {
                ...
                sftp.fstat(handle, (statErr, stats) => {
                    ...
                    if (stats.isDirectory()) {
                        sftp.close(handle, () => { });
                        reject(new types_1.FileSystemError(`Path is a directory: ${path}`, types_1.FileSystemErrorCodes.IS_DIRECTORY, path));
                        return;
                    }
    ...
    async listRecursive(options) {
        ...
        const pruneExpr = pruneNames.map((name) => `-name ${(0, shellEscape_1.quoteShellArg)(name)}`).join(' -o ');
        // Build find command: prune ignored dirs, print files (and optionally dirs)
        const typeFilter = includeDirs ? '' : '-type f';
        const command = [
            `find ${(0, shellEscape_1.quoteShellArg)(this.remotePath)}`,
            `\\( ${pruneExpr} \\) -prune -o`,
        ...
        try {
            ...
            for (const line of effectiveLines) {
                ...
                if (trimmed === this.remotePath || trimmed === this.remotePath + '/')
                    continue;
    ...
    async search(query, options) {
        ...
        const escapedPattern = (0, shellEscape_1.quoteShellArg)(searchPattern);
        // Build file extension filter if provided
        let includeFilter = '';
        if (options?.fileExtensions && options.fileExtensions.length > 0) {
            const extensions = options.fileExtensions.map((ext) => ext.startsWith('.') ? ext : `.${ext}`);
            includeFilter = extensions.map((e) => `--include=${(0, shellEscape_1.quoteShellArg)(`*${e}`)}`).join(' ');
        }
        // Use grep recursively with line numbers
        const command = `grep -rn ${caseFlag} ${includeFilter} -e ${escapedPattern} ${(0, shellEscape_1.quoteShellArg)(basePath)} 2>/dev/null | head -n ${maxResults}`;
        try {
            const result = await this.sshService.executeCommand(this.connectionId, command);
    ...
    async remove(path) {
        ...
        try {
            ...
            if (entry.type === 'dir') {
                ...
                const command = `rm -rf ${(0, shellEscape_1.quoteShellArg)(fullPath)}`;
                const result = await this.sshService.executeCommand(this.connectionId, command);
    ...
    async ensureConnected() {
        ...
        if (!connections.includes(this.connectionId)) {
            throw new types_1.FileSystemError('SSH connection not found', types_1.FileSystemErrorCodes.CONNECTION_ERROR, this.connectionId);
        }
    }
    /**
    ...
    resolveRemotePath(relPath) {
        ...
        if (normalized.startsWith('/')) {
            ...
            if (!this.isWithinBase(resolved)) {
                throw new types_1.FileSystemError('Path traversal detected: path escapes base directory', types_1.FileSystemErrorCodes.PATH_ESCAPE, relPath);
            }
            return resolved;
        }
        // Join with base path
    ...
    mapSftpError(error, path) {
        ...
        if (code === 2 || message.includes('No such file')) {
            return new types_1.FileSystemError(`File or directory not found: ${path || message}`, types_1.FileSystemErrorCodes.NOT_FOUND, path);
        }
        if (code === 3 || message.includes('Permission denied')) {
            return new types_1.FileSystemError(`Permission denied: ${path || message}`, types_1.FileSystemErrorCodes.PERMISSION_DENIED, path);
        }
        if (message.includes('is a directory')) {
            return new types_1.FileSystemError(`Path is a directory: ${path || message}`, types_1.FileSystemErrorCodes.IS_DIRECTORY, path);
        }
        if (message.includes('Not a directory')) {
            return new types_1.FileSystemError(`Path is not a directory: ${path || message}`, types_1.FileSystemErrorCodes.NOT_DIRECTORY, path);
        }
        if (message.includes('connection') || message.includes('Connection')) {
            return new types_1.FileSystemError(`Connection error: ${message}`, types_1.FileSystemErrorCodes.CONNECTION_ERROR, path);
        }
        // Default to unknown error
        return new types_1.FileSystemError(`Filesystem error: ${message}`, types_1.FileSystemErrorCodes.UNKNOWN, path);
    }
}

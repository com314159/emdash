...
exports.detectAndLoadTerminalConfig = detectAndLoadTerminalConfig;
 ...
 */
function detectAndLoadTerminalConfig() {
...
function detectMacOSTerminal() {
    ...
    if (iterm2Config) {
        logger_1.log.debug('terminalConfig:detected', { terminal: 'iTerm2' });
        return iterm2Config;
    }
    // Check Terminal.app
    const terminalAppConfig = loadTerminalAppConfig();
    if (terminalAppConfig) {
        logger_1.log.debug('terminalConfig:detected', { terminal: 'Terminal.app' });
        return terminalAppConfig;
    }
    // Check Alacritty
    const alacrittyConfig = loadAlacrittyConfig();
    if (alacrittyConfig) {
        logger_1.log.debug('terminalConfig:detected', { terminal: 'Alacritty' });
        return alacrittyConfig;
    }
    // Check Ghostty
    const ghosttyConfig = loadGhosttyConfig();
    if (ghosttyConfig) {
        logger_1.log.debug('terminalConfig:detected', { terminal: 'Ghostty' });
        return ghosttyConfig;
    }
    // Check Kitty
    const kittyConfig = loadKittyConfig();
    if (kittyConfig) {
        logger_1.log.debug('terminalConfig:detected', { terminal: 'Kitty' });
        return kittyConfig;
    }
    return null;
}
function detectWindowsTerminal() {
    ...
    try {
        ...
        if ((0, fs_1.existsSync)(settingsPath)) {
            ...
            if (config) {
                logger_1.log.debug('terminalConfig:detected', { terminal: 'Windows Terminal' });
                return config;
            }
        }
    }
    catch (error) {
        logger_1.log.warn('terminalConfig:windowsTerminal:readFailed', { error });
    }
    return null;
}
 ...
 */
function loadiTerm2Config() {
    try {
        const plistPath = (0, path_1.join)((0, os_1.homedir)(), 'Library', 'Preferences', 'com.googlecode.iterm2.plist');
        ...
        let jsonContent;
        try {
            jsonContent = (0, child_process_1.execSync)(`plutil -convert json -o - "${plistPath}"`, {
                encoding: 'utf8',
        ...
        logger_1.log.warn('terminalConfig:iTerm2:parseFailed', { error });
        return null;
    }
}
/**
 ...
 */
function loadiTerm2ConfigXML(plistPath) {
    try {
        const xmlContent = (0, fs_1.readFileSync)(plistPath, 'utf8');
 ...
 */
function parseiTerm2Color(colorObj) {
 ...
 */
function loadTerminalAppConfig() {
    try {
        const plistPath = (0, path_1.join)((0, os_1.homedir)(), 'Library', 'Preferences', 'com.apple.Terminal.plist');
        if (!(0, fs_1.existsSync)(plistPath)) {
            return null;
        }
        let jsonContent;
        ...
        logger_1.log.warn('terminalConfig:TerminalApp:parseFailed', { error });
        return null;
    }
}
/**
 ...
 */
function loadAlacrittyConfig() {
    try {
        const configPath = (0, path_1.join)((0, os_1.homedir)(), '.config', 'alacritty', 'alacritty.toml');
        if (!(0, fs_1.existsSync)(configPath)) {
            // Try YAML format (older versions)
            const yamlPath = (0, path_1.join)((0, os_1.homedir)(), '.config', 'alacritty', 'alacritty.yml');
        ...
        logger_1.log.warn('terminalConfig:Alacritty:parseFailed', { error });
        return null;
    }
}
/**
 ...
 */
function loadAlacrittyYAML(yamlPath) {
    try {
        const content = (0, fs_1.readFileSync)(yamlPath, 'utf8');
...
function loadGhosttyConfig() {
    try {
        const configPath = (0, path_1.join)((0, os_1.homedir)(), '.config', 'ghostty', 'config');
        if (!(0, fs_1.existsSync)(configPath)) {
            return null;
        }
        const content = (0, fs_1.readFileSync)(configPath, 'utf8');
        ...
        logger_1.log.warn('terminalConfig:Ghostty:parseFailed', { error });
        return null;
    }
}
/**
 ...
 */
function loadKittyConfig() {
    try {
        const configPath = (0, path_1.join)((0, os_1.homedir)(), '.config', 'kitty', 'kitty.conf');
        if (!(0, fs_1.existsSync)(configPath)) {
            return null;
        }
        const content = (0, fs_1.readFileSync)(configPath, 'utf8');
        ...
        logger_1.log.warn('terminalConfig:Kitty:parseFailed', { error });
        return null;
    }
}
/**
 ...
 */
function parseKittyConfig(content) {
    const theme = {};
    const lines = content.split('\n');
    for (const line of lines) {
        const trimmed = line.trim();
 ...
 */
function loadWindowsTerminalConfig(settingsPath) {
    try {
        const content = (0, fs_1.readFileSync)(settingsPath, 'utf8');
        ...
        if (colorSchemeName && config.schemes) {
            const scheme = config.schemes.find((s) => s.name === colorSchemeName);
            if (scheme) {
        ...
        logger_1.log.warn('terminalConfig:WindowsTerminal:parseFailed', { error });
        return null;
    }
}

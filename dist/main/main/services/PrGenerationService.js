...
exports.prGenerationService = exports.PrGenerationService = void 0;
...
class PrGenerationService {
     ...
     */
    async generatePrContent(taskPath, baseBranch = 'main', preferredProviderId) {
        try {
            ...
            if (preferredProviderId && this.isValidProviderId(preferredProviderId)) {
                tryOrder.push(preferredProviderId);
            }
            tryOrder.push('claude');
            for (const id of registry_1.PROVIDER_IDS) {
                tryOrder.push(id);
            }
            for (const providerId of tryOrder) {
                ...
                try {
                    ...
                    if (result) {
                        logger_1.log.info(`Generated PR content with ${providerId}`);
                        return {
                            title: result.title,
                    ...
                    logger_1.log.debug(`Provider ${providerId} generation failed, trying next`, { error });
                }
            }
     ...
     */
    async getGitContext(taskPath, baseBranch) {
        let diff = '';
        let commits = [];
        let changedFiles = [];
        try {
            ...
            try {
                ...
                try {
                    ...
                    logger_1.log.debug('Failed to fetch remote, continuing with existing refs', { fetchError });
                }
            }
            catch {
                // Remote doesn't exist, skip fetch
                logger_1.log.debug('Remote origin not found, skipping fetch');
            }
            ...
            if (baseBranchExists) {
                ...
                try {
                    const { stdout: diffOut } = await execFileAsync('git', ['diff', `${baseBranchRef}...HEAD`, '--stat'], { cwd: taskPath, maxBuffer: 10 * 1024 * 1024 });
                    diff = diffOut || '';
                    // Get list of changed files from commits
                    const { stdout: filesOut } = await execFileAsync('git', ['diff', '--name-only', `${baseBranchRef}...HEAD`], { cwd: taskPath });
                    ...
                    // Get commit messages
                    const { stdout: commitsOut } = await execFileAsync('git', ['log', `${baseBranchRef}..HEAD`, '--pretty=format:%s'], { cwd: taskPath });
                    commits = (commitsOut || '')
                        .split('\n')
                        .map((c) => c.trim())
                        .filter(Boolean);
                    ...
                    logger_1.log.debug('Failed to get diff/commits from base branch', { error });
                }
            }
            ...
            try {
                ...
                logger_1.log.debug('Failed to get working directory diff', { error });
            }
            // Fallback: if we still have no diff or commits, try staged changes
            if (commits.length === 0 && diff.length === 0) {
                try {
                    const { stdout: stagedDiff } = await execFileAsync('git', ['diff', '--cached', '--stat'], { cwd: taskPath, maxBuffer: 10 * 1024 * 1024 });
                    if (stagedDiff) {
                        diff = stagedDiff;
                        const { stdout: filesOut } = await execFileAsync('git', ['diff', '--cached', '--name-only'], { cwd: taskPath });
            ...
            logger_1.log.warn('Failed to get git context', { error });
        }
     ...
     */
    canUseForPrGeneration(providerId) {
     ...
     */
    async generateWithProvider(providerId, taskPath, diff, commits) {
        ...
        try {
            ...
            logger_1.log.debug(`Provider ${providerId} CLI not available`);
            return null;
        }
        ...
        for (let attempt = 0; attempt < 2; attempt++) {
            const { result, shouldRetry } = await this.spawnProvider(providerId, cliCommand, provider, taskPath, prompt);
            if (result)
                return result;
            if (!shouldRetry)
                break;
            logger_1.log.debug(`Retrying provider ${providerId} (attempt ${attempt + 2}/2) after parse failure`);
        }
        return null;
    }
    /**
     ...
     */
    spawnProvider(providerId, cliCommand, provider, taskPath, prompt) {
        return new Promise((resolve) => {
            const timeout = 60000;
            let stdout = '';
            let stderr = '';
            let resolved = false;
            const done = (result, shouldRetry) => {
                if (resolved)
                    return;
                resolved = true;
            ...
            const child = (0, child_process_1.spawn)(cliCommand, args, {
                cwd: taskPath,
            ...
            const timeoutId = setTimeout(() => {
                ...
                logger_1.log.debug(`Provider ${providerId} invocation timed out`);
            ...
            child.on('exit', (code, signal) => {
                ...
                if (code !== 0 && code !== null) {
                    logger_1.log.debug(`Provider ${providerId} exited with code ${code}`, { stderr });
                    done(null, false); // Don't retry on non-zero exit
                    return;
                }
                if (signal) {
                    logger_1.log.debug(`Provider ${providerId} killed by signal ${signal}`);
                    done(null, false);
                    return;
                }
                ...
                if (result) {
                    logger_1.log.info(`Successfully generated PR content with ${providerId}`);
                    done(result, false);
                }
                else {
                    logger_1.log.debug(`Failed to parse response from ${providerId}`, { stdout, stderr });
            ...
            child.on('error', (error) => {
                clearTimeout(timeoutId);
                logger_1.log.debug(`Failed to spawn ${providerId}`, { error });
     ...
     * Build prompt for PR generation
     */
    buildPrGenerationPrompt(diff, commits) {
  ...
  "description": "A well-structured markdown description using proper markdown formatting. Use ## for section headers, - or * for lists, \`code\` for inline code, and proper line breaks.\n\nUse actual newlines (\\n in JSON) for line breaks, not literal \\n text. Keep it straightforward and to the point."
    ...
    parseProviderResponse(response) {
        try {
            ...
            logger_1.log.debug('Failed to parse provider response', { error, response });
        }
        return null;
    }
    /**
     ...
     */
    generateHeuristicContent(diff, commits, changedFiles) {
     ...
     */
    generateFallbackContent(changedFiles) {
    ...
    isValidProviderId(id) {
        return registry_1.PROVIDER_IDS.includes(id);
    }
}
exports.PrGenerationService = PrGenerationService;
exports.prGenerationService = new PrGenerationService();

...
exports.SshConnectionMonitor = void 0;
 ...
 */
class SshConnectionMonitor extends events_1.EventEmitter {
    constructor(connectionChecker) {
        super();
        this.connections = new Map();
        this.DEFAULT_INTERVAL_MS = 30000; // 30 seconds
        this.MAX_RECONNECT_ATTEMPTS = 3;
        this.RECONNECT_BACKOFF_MS = [1000, 5000, 15000]; // Exponential backoff delays
     ...
     */
    startMonitoring(connectionId, config) {
     ...
     */
    updateState(connectionId, state, error) {
        const monitored = this.connections.get(connectionId);
        if (!monitored) {
            return;
        }
     ...
     */
    startHealthChecks(intervalMs = this.DEFAULT_INTERVAL_MS) {
        this.stopHealthChecks();
    ...
    async attemptReconnect(connectionId) {
        ...
        const delayIndex = Math.min(monitored.reconnectAttempts - 1, this.RECONNECT_BACKOFF_MS.length - 1);
...
exports.SshConnectionMonitor = SshConnectionMonitor;

...
exports.getProviderCustomConfig = getProviderCustomConfig;
exports.getAllProviderCustomConfigs = getAllProviderCustomConfigs;
exports.updateProviderCustomConfig = updateProviderCustomConfig;
...
function getPlatformTaskSwitchDefaults() {
    if (IS_MAC) {
        return {
...
const DEFAULT_SETTINGS = {
    ...
    projects: {
        defaultDirectory: (0, node_path_1.join)((0, node_os_1.homedir)(), 'emdash-projects'),
    },
    keyboard: {
...
function normalizeShortcutModifier(value, fallback) {
    if (typeof value !== 'string')
        return fallback;
...
function isBinding(binding, modifier, key) {
    return binding.modifier === modifier && binding.key === key;
}
function assertNoKeyboardShortcutConflicts(keyboard) {
    if (!keyboard)
        return;
    const seen = new Map();
    for (const [shortcutName, binding] of Object.entries(keyboard)) {
        ...
        if (conflictWith) {
            throw new Error(`Keyboard shortcut conflict: "${shortcutName}" duplicates "${conflictWith}".`);
        }
...
function persistSettings(settings) {
    try {
        ...
        (0, node_fs_1.writeFileSync)(file, JSON.stringify(settings, null, 2), 'utf8');
    }
    catch { }
}
/**
...
function normalizeSettings(input) {
    ...
    if (!prefix)
        prefix = DEFAULT_SETTINGS.repository.branchPrefix;
    if (prefix.length > 50)
    ...
    out.projectPrep.autoInstallOnOpenInEditor = Boolean(prep?.autoInstallOnOpenInEditor ?? DEFAULT_SETTINGS.projectPrep.autoInstallOnOpenInEditor);
    ...
    out.notifications = {
        ...
        osNotifications: Boolean(notif?.osNotifications ?? DEFAULT_SETTINGS.notifications.osNotifications),
        soundFocusMode: rawFocusMode === 'always' || rawFocusMode === 'unfocused'
            ? rawFocusMode
            : DEFAULT_SETTINGS.notifications.soundFocusMode,
    };
    // Default provider
    const defaultProvider = input?.defaultProvider;
    out.defaultProvider = (0, registry_1.isValidProviderId)(defaultProvider)
        ? defaultProvider
    ...
    let defaultDir = String(projects?.defaultDirectory ?? DEFAULT_SETTINGS.projects.defaultDirectory).trim();
    if (!defaultDir) {
        defaultDir = DEFAULT_SETTINGS.projects.defaultDirectory;
    }
    // Resolve ~ to home directory if present
    if (defaultDir.startsWith('~')) {
        defaultDir = (0, node_path_1.join)((0, node_os_1.homedir)(), defaultDir.slice(1));
    }
    out.projects = {
    ...
    const normalizeBinding = (binding, defaultBinding) => {
    ...
    const isLegacyArrowPair = isBinding(out.keyboard.nextProject, 'cmd', 'ArrowRight') &&
        isBinding(out.keyboard.prevProject, 'cmd', 'ArrowLeft');
    const isLegacyTabPair = isBinding(out.keyboard.nextProject, 'ctrl', 'Tab') &&
    ...
    out.interface = {
        autoRightSidebarBehavior: Boolean(iface?.autoRightSidebarBehavior ?? DEFAULT_SETTINGS.interface.autoRightSidebarBehavior),
    ...
    // Hidden Open In Apps
    const rawHidden = input?.hiddenOpenInApps;
    if (Array.isArray(rawHidden)) {
        const validated = rawHidden.filter(openInApps_1.isValidOpenInAppId);
 ...
 */
function getProviderCustomConfig(providerId) {
 ...
 */
function getAllProviderCustomConfigs() {
    const settings = getAppSettings();
 ...
 */
function updateProviderCustomConfig(providerId, config) {
    const settings = getAppSettings();

...
export class SshService extends EventEmitter {
    ...
    return new Promise<string>((resolve, reject) => {
      ...
      const timeout = setTimeout(() => {
        if (!settled) {
          ...
          reject(
            new Error(
              'SSH GSSAPI connection timed out. Check your Kerberos ticket (run kinit) and SSH config.'
            )
          );
        }
      }, 20000);
   ...
   */
  private async findExistingControlMaster(config: SshConfig): Promise<string | null> {
    try {
      // Use `ssh -G` to resolve the effective SSH config for this host
      const { execFile: execFileCb } = require('child_process');
      const resolvedConfig: string = await new Promise((resolve, reject) => {
        execFileCb(
          'ssh',
          ['-G', '-p', String(config.port), '-l', config.username, config.host],
      ...
      const checkResult: boolean = await new Promise((resolve) => {
        execFileCb(
          'ssh',
          [
            '-O',
